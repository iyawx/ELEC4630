...
for img in glob.iglob("*.jpg"):
    ...
    matches = face_recognition.compare_faces(known_faces, unknown_face_encoding,0.6)

    # Or instead, use the known face with the smallest distance to the new face
    face_distances = face_recognition.face_distance(known_faces, unknown_face_encoding)

    if img == "Me.jpg" or img == "unknown_test10.jpg" or img == "unknown_test9.jpg":
        actual = np.block([actual, 0])
        predictions = np.block([predictions, face_distances])
    else:
